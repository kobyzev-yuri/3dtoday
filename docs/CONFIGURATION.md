...
**Выбор модели:**
- **Легкая модель** (`qwen2.5:1.5b`): быстрая работа (~86 сек), подходит для большинства задач
- **Тяжелая модель** (`qwen3:8b`): медленная (>500 сек), но более качественная, для сложных задач
- В пользовательском интерфейсе можно выбирать любую доступную модель
