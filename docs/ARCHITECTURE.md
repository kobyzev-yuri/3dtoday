...
```mermaid
flowchart TD
    Start([Администратор]) --> InputMethod{Метод ввода}
    
    InputMethod -->|URL| URLParser[URL Parser]
    InputMethod -->|Файл| FileParser[File Parser]
    InputMethod -->|Ручной ввод| ManualInput[Manual Input]
    
    URLParser --> LLMAnalyzer[LLM URL Analyzer<br/>GPT-4o/Gemini]
    FileParser --> DocumentParser[Document Parser]
    ManualInput --> Validation[Валидация]
    
    LLMAnalyzer --> ParsedDoc[Распарсенный документ]
    DocumentParser --> ParsedDoc
    
    ParsedDoc --> LibrarianAgent[KB Librarian Agent]
    
    LibrarianAgent --> RelevanceCheck[Проверка релевантности]
    RelevanceCheck -->|Релевантно| MetadataExtraction[Извлечение метаданных]
    RelevanceCheck -->|Не релевантно| Reject[Отклонение]
    
    MetadataExtraction --> Summary[Генерация summary]
    Summary --> AdminReview[Проверка администратором]
    
    AdminReview -->|Одобрено| Indexer[Article Indexer]
    AdminReview -->|Отклонено| Reject
    
    Indexer --> EmbeddingGen[Генерация эмбеддингов]
    EmbeddingGen --> VectorDB[(Qdrant<br/>Vector DB)]
    
    VectorDB --> Success([Статья добавлена])
