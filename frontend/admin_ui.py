...
if input_method == "🤖 По URL (через LLM - GPT-4o/Gemini)":
    ...
    st.warning("⚠️ **Совет**: Если обычный парсинг не работает (например, для MatterHackers, All3DP, Prusa KB), используйте метод '🤖 По URL (через LLM)' - он лучше справляется с проблемными сайтами.")
