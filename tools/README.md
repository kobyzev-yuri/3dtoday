# Инструменты для тестирования и автоматизации работы с KB

Эта папка содержит скрипты для тестирования и автоматизации работы с базой знаний (KB).

## 📋 Доступные инструменты

### Тестирование

#### `test_article_parse.py`
Тестирование парсинга статей через различные LLM провайдеры (GPT-4o, Gemini).

**Использование:**
```bash
python3 tools/test_article_parse.py
```

**Что делает:**
- Тестирует парсинг статьи через OpenAI (GPT-4o) и Gemini
- Сравнивает результаты разных провайдеров
- Сохраняет результаты в JSON файлы

#### `test_gemini3_kb_formation.py`
Тестирование формирования базы знаний через ProxyAPI Gemini 3.

**Использование:**
```bash
python3 tools/test_gemini3_kb_formation.py "https://3dtoday.ru/..."
```

**Что делает:**
- Анализирует статью через Gemini 3 Pro
- Извлекает метаданные (проблема, решения, принтеры, материалы)
- Проверяет качество формирования KB

#### `check_test_results.sh`
Проверка результатов тестирования.

**Использование:**
```bash
bash tools/check_test_results.sh
```

**Что делает:**
- Проверяет наличие файлов с результатами тестов
- Выводит статистику по тестам

### Автоматизация работы с KB

#### `check_kb_stats.py`
Проверка статистики базы знаний.

**Использование:**
```bash
python3 tools/check_kb_stats.py
```

**Что делает:**
- Показывает статистику KB через API
- Показывает статистику напрямую через Qdrant
- Выводит количество статей, изображений, векторов

#### `view_article.py`
Просмотр статьи из KB по ID.

**Использование:**
```bash
python3 tools/view_article.py <article_id>
```

**Что делает:**
- Получает статью из KB по ID
- Выводит метаданные и содержимое
- Поддерживает правильную кодировку UTF-8

**Пример:**
```bash
python3 tools/view_article.py stringing_pla_ender3
```

## 🔧 Требования

Все скрипты требуют:
- Запущенный FastAPI сервер (`http://localhost:8000`)
- Настроенный `config.env` с API ключами (если используются внешние LLM)

#### `test_user_ui_functionality.py`
Тесты функционала пользовательского интерфейса (техподдержка).

**Использование:**
```bash
# Запустить все тесты
python3 tools/test_user_ui_functionality.py

# Только реалистичный сценарий
python3 tools/test_user_ui_functionality.py --scenario

# Тест сохранения контекста
python3 tools/test_user_ui_functionality.py --context

# Тест потока уточнений
python3 tools/test_user_ui_functionality.py --clarification
```

**Что проверяет:**
- ✅ Реалистичный сценарий диалога с пользователем
- ✅ Уточняющие вопросы и ответы на них
- ✅ Сохранение контекста между запросами
- ✅ Загрузку изображений дефектов (если реализовано)
- ✅ Работу с историей диалога

**Примеры:**
```bash
# Полный тест с реалистичным сценарием
python3 tools/test_user_ui_functionality.py --scenario

# Комбинация тестов
python3 tools/test_user_ui_functionality.py --scenario --context
```

#### `test_admin_ui_functionality.py`
Комплексные тесты функционала веб-интерфейса администрирования KB.

**Использование:**
```bash
# Запустить все тесты
python3 tools/test_admin_ui_functionality.py

# Только тест JSON
python3 tools/test_admin_ui_functionality.py --json

# Тест PDF (требуется указать путь)
python3 tools/test_admin_ui_functionality.py --pdf path/to/file.pdf

# Тест URL
python3 tools/test_admin_ui_functionality.py --url "https://3dtoday.ru/..."

# Комбинация тестов
python3 tools/test_admin_ui_functionality.py --json --url "https://3dtoday.ru/..."
```

**Что проверяет:**
- ✅ Загрузка JSON формата через `/api/kb/articles/parse`
- ✅ Загрузка PDF формата с картинками через `/api/kb/articles/parse`
- ✅ Загрузка URL через `/api/kb/articles/parse` и `/api/kb/articles/parse_with_llm`
- ✅ Валидация структуры данных
- ✅ Анализ через агента-библиотекаря
- ✅ Извлечение метаданных

**Примеры:**
```bash
# Тест с JSON файлом из test_data
python3 tools/test_admin_ui_functionality.py --json tools/test_data/sample_article.json

# Тест с PDF URL
python3 tools/test_admin_ui_functionality.py --pdf "https://example.com/manual.pdf"

# Тест с конкретным URL
python3 tools/test_admin_ui_functionality.py --url "https://3dtoday.ru/blogs/..."
```

## 📚 Дополнительная информация

- Подробнее о работе с KB: см. `docs/KB_MANAGEMENT.md`
- Подробнее о тестировании: см. документацию в соответствующих скриптах




